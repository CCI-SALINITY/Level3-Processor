...
output_dir=('/net/nfs/tmp15/chakroun/L3_output/Level3_intermediate/SSS_SMAP_merged/');
...
for orb=['A' 'D']
	for ii=1:length(ndate)% tous les 1er et 15 du mois
		...
		for uu=1:length(Date_kk)
			fic=dir([input_dir,'smapL2corrected_',Date_kk(uu,1:8),'_',orb,'.mat']);
			if (length(fic)>0)
				SSS_corr=[];
				totalcorrection=[];
				SSS_random=[];
		...
		if (cmpt>0)
			...
			SSScorrigee=reshape(SSScorrigee,avg*2,nlon,nlat);
			bias=reshape(bias,avg*2,nlon,nlat);
			randomerror=reshape(randomerror,avg*2,nlon,nlat);
			%filtre 3 sigma
			...
			%noutliers
			noutliers_smap=zeros(nlon,nlat);
			for nln=1:nlon
				for nlt=1:nlat
					UU=[];
					UU=find((SSScorrigee(:,nln,nlt)>=0)&(SSScorrigee(:,nln,nlt)<=100));
					noutliers_smap(nln,nlt)=length(UU);
				end
			end
			...
			sigma=sqrt(SSS_smap_random.^2+sss_erreur_L4.^2);%erreur corrigee
			...
			output_file=([output_dir,'/monthly_',orb,'/smapL3_monthlyaveraged_',Datecentral,'centred_',orb])
