...
output_dir=('/net/nfs/tmp15/chakroun/L3_output/Level3_intermediate/SSS_SMOS_merged/');
...
for orb=['A' 'D']
	for ii=1:length(ndate)% tous les 1er et 15 du mois
		SSScorrigee=nan(avg,nlon,nlat);
		randomerror=nan(avg,nlon,nlat);
		noutliers_L4_smos=zeros(nlon,nlat);
		bias=nan(avg,nlon,nlat);
		Date_kk=[];
		Date_kk=datestr(ndate(ii)-15:ndate(ii)+15,30);%les dates d un mois
		cmpt=0;
		for uu=1:length(Date_kk)
			fic=dir([input_dir,'smosL2corrected_',Date_kk(uu,1:8),'_',orb,'.mat']);
			if (length(fic)>0)
				SSS_corr=[];
				totalcorrection=[];
				SSS_random=[];
		...
		if (cmpt>0)
			...
			%noutliers
			noutliers_smos=zeros(nlon,nlat);
			for nln=1:nlon
				for nlt=1:nlat
					UU=[];
					UU=find((SSScorrigee(:,nln,nlt)>=0)&(SSScorrigee(:,nln,nlt)<=100));
					noutliers_smos(nln,nlt)=length(UU);
				end
			end
			...
			output_file=([output_dir,'/monthly_',orb,'/smosL3_monthlyaveraged_',Datecentral,'centred_',orb]);
...
subplot(2,2,3)
pcolor(lon_ease,lat_ease,SSS_smos_bias')
hold on
plot(long,lat)
shading flat
colorbar
